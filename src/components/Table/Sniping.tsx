...
import { SnipingState } from '../../interfaces'
import Button from '../Button'
import Buttons from '../Buttons'

const TableSniping = ({ data }) => {
  const perPage = 15
  ...
  return (
    <>
      <table>
        ...
        <tbody>
          {clientsPaginated.map((state: SnipingState) => (
            <tr key={state.created}>
              <td data-label="created">{state.created}</td>
              <td data-label="activity">
                {state.poolId.replace(/(.{10})(.*?)(.{10})$/, '$1...$3')}
              </td>
              <td data-label="baseAmount">
                {state.tokenAddress.replace(/(.{10})(.*?)(.{10})$/, '$1...$3')}
              </td>
